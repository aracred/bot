...
describe('signup parameters parser 📝', () => {
  ...
  test('It errors on empty strings', () =>
    expect(() => parseSignup({ contents: '', author })).toThrow(/empty/))

  test('It throws if no arguments are provided', () =>
    expect(() => parseSignup({ contents: '!ac signup', author })).toThrow(/arguments/))

  test('It throws if it receives undefined as discordId', () =>
    expect(() => parseSignup({ contents: '!ac signup foo/123', author: {} })).toThrow(/no Discord ID/))

  test('It properly parses and returns the parameters for signup', () =>
    expect(parseSignup({ contents: '!ac signup github/foo', author })).toEqual(['github/foo', `discord/${discordId}`]))
})
